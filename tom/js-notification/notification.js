...
/* Example
        ...
        jQuery.ajax({
            method: "POST",
            url: DASHBOARD_CONTENT.root + '/my-endpoint',
            dataType: "JSON",
            data: new FormData(form),
            processData: false,
            contentType: false,
            beforeSend: function ( xhr ) {
                xhr.setRequestHeader( 'X-WP-Nonce', DASHBOARD_CONTENT.nonce );
            },
            success : function( response ) {
                if(response.status == "success"){
                    jQuery(`${response.container}`).replaceWith(response.html);
                    triggerNotification('Success', "All Locations have been Updated", 'nfc-top-right', 'success' );
                } else if(response.status == "failed"){
                    console.log( response );
                    triggerNotification('Error', "Something Went Wrong", 'nfc-top-right', 'error' );
                }
                _doing_bulk_action_ajax = false;
            },
